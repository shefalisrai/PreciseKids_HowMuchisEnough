function TestretestReliability_FullCurves_NetworkWise_Child(network_number, network_name, dtseries_C_sessions_DORA, dtseries_C_sessions_RX, dtseries_C_sessions_YT)
%TestretestReliability_FCTRC_Networkwise_AllTasks for Children
...
child_networkconsensus_map=ciftiopen('/Users/shefalirai/Desktop/PK_networkassignment/Thirdconsensus_adultchildcombined_templatematching_66percent_networkconsensusmap.dscalar.nii', wbcommand);
child_networkconsensus=child_networkconsensus_map.cdata;

dmn_vertices=[];

% Find the DMN vertices 
dmn_vertices = find(child_networkconsensus(:,1) == network_number); %between 1 and 16 for networks, make sure to ignore network 4 and 6 as they do not exist

% Define the cell arrays corresponding to each variable
variables = {dtseries_C_sessions_DORA, dtseries_C_sessions_YT, dtseries_C_sessions_RX};

% Initialize cell arrays to store the extracted matrices for each variable
DMN_C_DORA = cell(size(dtseries_C_sessions_DORA));
DMN_C_YT = cell(size(dtseries_C_sessions_YT));
DMN_C_RX = cell(size(dtseries_C_sessions_RX));
...
for v = 1:numel(variables)
    ...
    for i = 1:numel(variables{v})
        ...
        switch v
            case 1
                DMN_C_DORA{i} = extracted_matrix;
            case 2
                DMN_C_YT{i} = extracted_matrix;
            case 3
                DMN_C_RX{i} = extracted_matrix;
        end
    end
end
...
DMN_C_combined1 = cell(26, 1); % Combining sessions 1 and 4 across all tasks
DMN_C_combined2 = cell(26, 1); % Combining sessions 2 and 3 across all tasks
...
for sub = subject_numbers
    ...
    for task = tasks
        ...
        data_variable_name_C = sprintf('DMN_C_%s', task{1});
    ...
    DMN_C_combined1{sub} = temp_C_combined1;
    DMN_C_combined2{sub} = temp_C_combined2;
end
...
scan_length_max_C = cell(1, numel(DMN_C_combined1));

for i = 1:numel(DMN_C_combined1)
    % Get the size of the matrices in the current cells 
    size_firstfourth_C = size(DMN_C_combined1{i});
    size_secondthird_C = size(DMN_C_combined2{i});
    
    % Compare the column sizes and store the matrix with the largest column size
    if size_firstfourth_C(2) >= size_secondthird_C(2)
        scan_length_max_C{i} = size(DMN_C_combined1{i},2);
    else
        scan_length_max_C{i} = size(DMN_C_combined2{i},2);
    end
end
...
reliability_scanlength_cell_C = cell(1, numel(DMN_C_combined1));

% Outer loop for each subject
for j = 1:numel(DMN_C_combined1)
    % Check if both first and last halves are not empty
    if ~isempty(DMN_C_combined1{j}) && ~isempty(DMN_C_combined2{j})
        % Calculate scan_length_max for the current subject
        scan_length_max_C_subject = max(size(DMN_C_combined1{j}, 2), size(DMN_C_combined2{j}, 2));
        ...
        for i = scan_length_increment:scan_length_increment:scan_length_max_C_subject
            ...
            DMN_firstfourth_columns_C = DMN_C_combined1{j}(:, 1:min(i, size(DMN_C_combined1{j}, 2)));
            DMN_secondthird_columns_C = DMN_C_combined2{j}(:, 1:min(i, size(DMN_C_combined2{j}, 2)));

            % Compute correlations for the current subject
            DMN_firstfourth_connectome_C = corr(DMN_firstfourth_columns_C');
            DMN_secondthird_connectome_C = corr(DMN_secondthird_columns_C');

            DMN_corr_allses_C = zeros(1, size(DMN_firstfourth_connectome_C, 1));

            % Remove NaNs
            DMN_firstfourth_connectome_C(isnan(DMN_firstfourth_connectome_C)) = 0;
            DMN_secondthird_connectome_C(isnan(DMN_secondthird_connectome_C)) = 0;

            % Correlate
            for r = 1:size(DMN_firstfourth_connectome_C, 1)
               DMN_corr_allses_C(r) = corr(DMN_firstfourth_connectome_C(r, :)', DMN_secondthird_connectome_C(r, :)');
            end

            % Store the mean correlation for the current scan length
            reliability_scanlength_C(i/scan_length_increment) = nanmean(DMN_corr_allses_C);
...
matched_matrix_C = nan(numel(reliability_scanlength_cell_C), min_length_C);
...
csvwrite(strcat(FCTRC_outputpath, sprintf('/Child_FCTRC_%s_allscantimes_meanvalues.csv', network_name)), mean_values_C)
csvwrite(strcat(FCTRC_outputpath, sprintf('/Child_FCTRC_%s_allscantimes_stdvalues.csv', network_name)), std_values_C)
