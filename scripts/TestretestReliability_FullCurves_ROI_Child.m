function TestretestReliability_FullCurves_ROI_Child(ROI_number, dtseries_C_sessions_DORA, dtseries_C_sessions_RX, dtseries_C_sessions_YT)
...
%assumed network order in conc file =  {'ROI','Vis','FP','DAN','VAN','Sal','CO','SMd','SML','AUD', 'Tpole', 'MTL','PMN','PON'};
...
ROI_C_DORA = cell(size(dtseries_C_sessions_DORA));
ROI_C_YT = cell(size(dtseries_C_sessions_YT));
ROI_C_RX = cell(size(dtseries_C_sessions_RX));
...
for v = 1:numel(variables)
    ...
    for i = 1:numel(variables{v})
        ...
        switch v
            case 1
                ROI_C_DORA{i} = extracted_matrix;
            case 2
                ROI_C_YT{i} = extracted_matrix;
            case 3
                ROI_C_RX{i} = extracted_matrix;
        end
    end
end
...
ROI_C_combined1 = cell(26, 1); % Combining sessions 1 and 4 across all tasks
ROI_C_combined2 = cell(26, 1); % Combining sessions 2 and 3 across all tasks
...
for sub = subject_numbers
    ...
    for task = tasks
        ...
        data_variable_name_C = sprintf('ROI_C_%s', task{1});
    ...
    ROI_C_combined1{sub} = temp_C_combined1;
    ROI_C_combined2{sub} = temp_C_combined2;
end
...
scan_length_max_C = cell(1, numel(ROI_C_combined1));

for i = 1:numel(ROI_C_combined1)
    % Get the size of the matrices in the current cells 
    size_firstfourth_C = size(ROI_C_combined1{i});
    size_secondthird_C = size(ROI_C_combined2{i});
    
    % Compare the column sizes and store the matrix with the largest column size
    if size_firstfourth_C(2) >= size_secondthird_C(2)
        scan_length_max_C{i} = size(ROI_C_combined1{i},2);
    else
        scan_length_max_C{i} = size(ROI_C_combined2{i},2);
    end
end
...
reliability_scanlength_cell_C = cell(1, numel(ROI_C_combined1));

% Outer loop for each subject
for j = 1:numel(ROI_C_combined1)
    % Check if both first and last halves are not empty
    if ~isempty(ROI_C_combined1{j}) && ~isempty(ROI_C_combined2{j})
        % Calculate scan_length_max for the current subject
        scan_length_max_C_subject = max(size(ROI_C_combined1{j}, 2), size(ROI_C_combined2{j}, 2));
        ...
        for i = scan_length_increment:scan_length_increment:scan_length_max_C_subject
            ...
            ROI_firstfourth_columns_C = ROI_C_combined1{j}(:, 1:min(i, size(ROI_C_combined1{j}, 2)));
            ROI_secondthird_columns_C = ROI_C_combined2{j}(:, 1:min(i, size(ROI_C_combined2{j}, 2)));

            % Compute correlations for the current subject
            ROI_firstfourth_connectome_C = corr(ROI_firstfourth_columns_C');
            ROI_secondthird_connectome_C = corr(ROI_secondthird_columns_C');

            ROI_corr_allses_C = zeros(1, size(ROI_firstfourth_connectome_C, 1));

            % Remove NaNs
            ROI_firstfourth_connectome_C(isnan(ROI_firstfourth_connectome_C)) = 0;
            ROI_secondthird_connectome_C(isnan(ROI_secondthird_connectome_C)) = 0;

            % Correlate
            for r = 1:size(ROI_firstfourth_connectome_C, 1)
               ROI_corr_allses_C(r) = corr(ROI_firstfourth_connectome_C(r, :)', ROI_secondthird_connectome_C(r, :)');
            end

            % Store the mean correlation for the current scan length
            reliability_scanlength_C(i/scan_length_increment) = nanmean(ROI_corr_allses_C);
...
csvwrite(strcat(ROIs_FCTRCs_outputpath, sprintf('/Child_FCTRC_%dROI_allscantimes_meanvalues.csv', ROI_number)), mean_values_C)
csvwrite(strcat(ROIs_FCTRCs_outputpath, sprintf('/Child_FCTRC_%dROI_allscantimes_stdvalues.csv', ROI_number)), std_values_C)
