function TestretestReliability_FullCurves_AllROIsforNetwork_Parent(network_number, network_name)
...
%Run this script after AveragedFC_143ROIs_EachNetwork_Connectomes.m
...
% Load network data
load('EachNetwork_AveragedROI_Timeseries_Parent_Session1and4.mat')
load('EachNetwork_AveragedROI_Timeseries_Parent_Session2and3.mat')


% For the specified network, open corresponding ROIs for that network from 1/4 and 2/3 sessions
ROI_P_combined1 = network_data_1and4{1, network_number};
ROI_P_combined2 = network_data_2and3{1, network_number};

% Specify the scan length increment
scan_length_increment = 30;

% Specify the maximum scan length
scan_length_max_P = size(ROI_P_combined2{1}, 2);

% Iterate over each subject
for j = 1:size(ROI_P_combined1{1}, 1)
    % Inner loop through increments for the scan length
    for i = scan_length_increment:scan_length_increment:scan_length_max_P
        % Ensure the loop index is within bounds for scan_length_max
        if i > scan_length_max_P
            break;
        end
        ...
        for roi = 1:numel(ROI_P_combined1)
            combined_ROI_session1 = [combined_ROI_session1; ROI_P_combined1{roi}(j, 1:i)];
            combined_ROI_session2 = [combined_ROI_session2; ROI_P_combined2{roi}(j, 1:i)];
        end

        % Compute correlation matrices
        ROI_P_combined1_connectome{(i/scan_length_increment), j} = corr(combined_ROI_session1');
        ROI_P_combined2_connectome{(i/scan_length_increment), j} = corr(combined_ROI_session2');
...
csvwrite(strcat(ROIs_FCTRCs_outputpath, sprintf('/Parent_FCTRC_%snetwork_allscantimes_stdvalues.csv', network_name)), std_values_P)
