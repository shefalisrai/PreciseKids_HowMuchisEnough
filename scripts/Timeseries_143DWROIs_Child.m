function Timeseries_143DWROIs_Child(ROI_number, scan_length_volumes, dtseries_C_sessions_DORA, dtseries_C_sessions_RX, dtseries_C_sessions_YT)
...
ROI_C_Timeseries1 = cell(26, 1); % Combining sessions 1 and 4 across all tasks
ROI_C_Timeseries2 = cell(26, 1); % Combining sessions 2 and 3 across all tasks
...
for sub = subject_numbers
    ...
    ROI_C_Timeseries1{sub,1} = temp_C_combined1;
    ROI_C_Timeseries2{sub,1} = temp_C_combined2;
end


% Initialize the matrix to store the averaged values
num_columns = scan_length_volumes; % 150 volumes is 5 minutes
num_cells = size(ROI_C_Timeseries1, 1);
ROI_C_Timeseries1_averaged = zeros(num_cells, num_columns);
ROI_C_Timeseries2_averaged = zeros(num_cells, num_columns);


% Iterate through each cell in the cell array
for i = 1:num_cells
    % Check if the cell is empty (for row 1 and 3)
    if isempty(ROI_C_Timeseries1{i})
        continue; % Skip to the next iteration
    end
    
    % Calculate the mean of each column across all rows
    mean_values1 = mean(ROI_C_Timeseries1{i}(:, 1:num_columns), 1);
    mean_values2 = mean(ROI_C_Timeseries2{i}(:, 1:num_columns), 1);

    % Store the mean values in the new matrix
    ROI_C_Timeseries1_averaged(i, :) = mean_values1;
    ROI_C_Timeseries2_averaged(i, :) = mean_values2;
end


% Remove rows 1 and 3 since missing subjects
ROI_C_Timeseries1_averaged_final=ROI_C_Timeseries1_averaged([2,4:end],:); %sessions 1 and 4
ROI_C_Timeseries2_averaged_final=ROI_C_Timeseries2_averaged([2,4:end],:); %sessions 2 and 3


%Save mean Timeseries ROI values for each network
csvwrite(strcat(ROIs_outputpath, sprintf('/Child_Timeseries_%dROI_sessions1and4.csv', ROI_number)), ROI_C_Timeseries1_averaged_final)
csvwrite(strcat(ROIs_outputpath, sprintf('/Child_Timeseries_%dROI_sessions2and3.csv', ROI_number)), ROI_C_Timeseries2_averaged_final)
